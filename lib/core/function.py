...
def train(config, train_loader, dataset, converter, model, criterion, optimizer, device, epoch, writer_dict=None, output_dict=None):
    ...
    proc = tqdm.tqdm(total=len(dataset), desc=f'Epoch {epoch}', unit='img')
    # end = time.time()
    for i, (inp, idx) in enumerate(train_loader):
        ...
        if i % config.PRINT_FREQ == 0:
            ...
            # print(msg)
            proc.set_postfix(loss='{loss.val:.5f} ({loss.avg:.5f})'.format(loss=losses))

            if writer_dict:
                writer = writer_dict['writer']
                global_steps = writer_dict['train_global_steps']
                # writer.add_scalar('train_loss', losses.avg, global_steps)
                writer.line(X=torch.IntTensor([global_steps]),
                    Y=torch.Tensor([losses.avg]),
                    win='train_loss', update=None if global_steps == 0 else 'append', opts=dict(title='train_loss'))
...
def validate(config, val_loader, dataset, converter, model, criterion, device, epoch, writer_dict, output_dict):
    ...
    proc = tqdm.tqdm(total=len(dataset), desc=f'Test aftrer epoch {epoch}', unit='img')
    with torch.no_grad():
    ...
    if writer_dict:
        ...
        writer.line(X=torch.IntTensor([global_steps]),
            ...
            win='valid_acc', update=None if global_steps == 0 else 'append', opts=dict(title='valid_acc'))
